// MultiX v21 — agregar/editar equipos por centro, incluyendo marca, modelo y serie
(function(){
  'use strict';
  ...
  function installStyles(){
    ...
    const s=document.createElement('style');s.id='mxEqManagerV21Styles';s.textContent=`
      .mx-eq-toolbar{display:flex;justify-content:flex-end;margin:0 0 12px}.mx-eq-add{background:#55d8ca;color:#09202a;border:0;border-radius:10px;padding:11px 16px;font-weight:900;cursor:pointer}
  ...
  window.mxAddCenterEquipment=function(){
    ...
    data.centers[currentCenter].equipment.push(newEquipment());persist();renderEqEditor();
    setTimeout(()=>{const cards=document.querySelectorAll('.mx-eq-card');cards[cards.length-1]?.scrollIntoView({behavior:'smooth',block:'center'});},50);
  };
  window.mxDeleteCenterEquipment=function(i){
    ...
    list.splice(i,1);persist();renderEqEditor();
  };
  ...
  function render(){
    ...
    host.innerHTML=`<div class="mx-eq-toolbar"><button type="button" class="mx-eq-add" onclick="mxAddCenterEquipment()">+ Agregar equipo</button></div>`+
      (list.length?list.map((e,i)=>{const interval=recalc(e);return `<div class="mx-eq-card">
      ...
      </div>`;}).join(''):'<div class="empty">Sin equipos registrados en este centro.</div>');
  }
