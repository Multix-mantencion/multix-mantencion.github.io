// MultiX v45 — asegurar Centro Delta en la lista de centros activos, sin alterar otros datos
(function(){
'use strict';
function persist(){try{localStorage.setItem('multixMantencion',JSON.stringify(data));}catch(_){}}
function ensureDelta(){
  if(typeof data==='undefined'||!data?.centers?.Delta)return false;
  ...
  try{
    if(typeof rCenter!=='undefined'&&rCenter&&!Array.from(rCenter.options||[]).some(o=>o.value==='Delta')){
      const areaIndex=Array.from(rCenter.options||[]).findIndex(o=>o.value==='Área / Base Cisnes');
      const opt=new Option('Delta','Delta');
...
let n=0;const t=setInterval(()=>{n++;if(ensureDelta()||n>120)clearInterval(t);},100);
})();
