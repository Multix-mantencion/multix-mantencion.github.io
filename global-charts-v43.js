// MultiX v48 — gráficos históricos acumulados: globales o por área según selector
(function(){
'use strict';
...
function selectedZone(){const z=$('mxeZoneArea')?.value||$('summary')?.dataset?.mxeArea||'Todas las zonas';return['Todas las zonas','Área Puyuhuapi','Área Refugio'].includes(z)?z:'Todas las zonas';}
function centersFor(zone){const all=centerNames();if(zone==='Área Refugio')return all.filter(n=>REFUGIO_SET.has(norm(n)));if(zone==='Área Puyuhuapi')return all.filter(n=>!REFUGIO_SET.has(norm(n)));return all;}
...
function records(zone){
 const allowed=new Set(centersFor(zone)),wm=new Map(),fm=new Map();
...
function build(zone){
 const rec=records(zone),names=centersFor(zone);
...
function signature(zone,d){return JSON.stringify({zone,f:d.centerCounts,e:d.equipRank,w:d.workRows});}
function patch(){
 const summary=$('summary'),shell=summary?.querySelector('.mxe-shell');if(!summary||!shell)return;
 const zone=selectedZone(),d=build(zone),sig=signature(zone,d);if(shell.dataset.mxGlobalChartsSig===sig)return;
...
function start(){const summary=$('summary');if(!summary)return setTimeout(start,100);new MutationObserver(schedule).observe(summary,{childList:true,subtree:true});summary.addEventListener('change',e=>{if(e.target?.id==='mxeZoneArea')schedule();},true);window.addEventListener('storage',schedule);setInterval(schedule,1200);window.mxRefreshGlobalChartsV43=schedule;schedule();}
