// MultiX v15 — asignar equipos de Área / Back Up / sin centro a un centro
(function(){
  'use strict';
  ...
  function purgePreviouslyAssigned(){
    ensureTracking();
    const done=new Set(data.settings.assignedAreaAssets.map(String));
  ...
  function installStyles(){
    ...
    const s=document.createElement('style');s.id='mxAssignAreaStyles';s.textContent=`
      ...
      .area-assign-btn{white-space:nowrap}.area-assigned-empty{padding:18px;text-align:center;color:var(--muted);border:1px dashed var(--line);border-radius:12px}
      @media(max-width:760px){.area-table{min-width:980px}.area-assign-select{min-width:155px}}
  ...
  function render(){
    ...
    areaAssetList.innerHTML=`<div class="tablewrap"><table class="area-table"><thead><tr><th>Grupo</th><th>Equipo</th><th>Actual</th><th>Últ. mant.</th><th>Próxima</th><th>Estado</th><th>Detalle</th><th>Asignar a</th><th>Acción</th></tr></thead><tbody>${rows.map((e,i)=>`<tr>
      ...
      <td><button class="btn small primary area-assign-btn" onclick="mxAssignAreaAsset(${i})">Asignar</button></td>
  ...
  function install(){
    ...
    installStyles();ensureTracking();purgePreviouslyAssigned();persist();
    window.renderAreaAssets=render;
    render();
    return true;
  }
